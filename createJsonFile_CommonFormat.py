...
def getHistoContentInJson(histo):
    ...
    if (histo.GetYaxis().GetNbins()==1):
        ...
        for i in range(1,histo.GetXaxis().GetNbins()+1):
            ...
            xBinValue=histoName+":[%.2f,%.2f]" %(histo.GetXaxis().GetBinLowEdge(i), histo.GetXaxis().GetBinUpEdge(i))
        ...
        for i in range(1,histo.GetXaxis().GetNbins()+1):
            yBins={}
            #xBinValue=str(histo.GetXaxis().GetBinLowEdge(i))+","+str(histo.GetXaxis().GetBinUpEdge(i))
            xBinValue="abseta:[%.2f,%.2f]" %(histo.GetXaxis().GetBinLowEdge(i), histo.GetXaxis().GetBinUpEdge(i))
            for j in range(1,histo.GetYaxis().GetNbins()+1):
                yBinValue="pt:[%.2f,%.2f]" %(histo.GetYaxis().GetBinLowEdge(j), histo.GetYaxis().GetBinUpEdge(j))
...
while (key): #loop
    ...
    while (subkey):
            ...
            while (subsubkey):
                if "histo"  in subsubkey.GetName() or "abseta_pt_DATA" in subsubkey.GetName():
                    ...
                    if "abseta_pt" in subsubkey.GetName():
                        efficienciesForThisID["abseta_pt"] = getHistoContentInJson(theHistoPlot)
        ...
        if "ratio" in subkey.GetName() and 'abseta_pt' in subkey.GetName():
            efficienciesForThisID = {}
